#!/usr/bin/env bash
# Fresh 0810 Stage1: MSE loss only, halved limb blob radii (no sigmoid / no pixel loss).
set -euo pipefail
...
exec >>"$LOG" 2>&1
echo "[$(date '+%F %T')] ===== 0810 FRESH STAGE1 (MSE loss + limb-half blobs) ====="
...
"$CAMTEST" scripts/train_heatmap.py \
  ...
  --epochs 150 --batch-size 32 --workers 8 --lr 0.0001 --weight-decay 0.005 \
...
echo "[$(date '+%F %T')] ===== 0810 FRESH STAGE1 DONE ====="
