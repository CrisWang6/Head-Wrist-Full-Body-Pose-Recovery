#!/usr/bin/env bash
# Resume weekend master after Task2 completed (master crashed post-ankle).
set -euo pipefail
...
LOG="$DATASET/logs/weekend_master_resume.log"
...
run_stage1() {
  ...
  "$CAMTEST_PY" scripts/train_heatmap.py \
    ...
    --joint-radius-config "$RADIUS_CFG" --default-joint-radius-px 10 --early-stop-patience 20 \
    --save-every 5 --keep-last 3 --log-every 50 --prefetch-factor 4 --max-hours 72
...
set_status "done"
log "===== WEEKEND MASTER RESUME DONE ====="
