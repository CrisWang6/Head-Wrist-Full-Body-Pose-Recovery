"""Run Stage3 aligned train-set wu limb eval + pred/GT dual skeleton viz on gwj."""
...
OUT_REMOTE = f"{DS}/eval/v31/stage3_aligned_train_wu_3d_viz"
LOCAL_OUT = Path(__file__).resolve().parent / "output" / "stage3_v31_aligned_train_wu_3d_viz"
...
REMOTE_SH = textwrap.dedent(
    ...
    echo "=== Stage3 aligned train inference (wu limb) ==="
    ...
    {CAMTEST} {JP}/render_stage3_dual_skeleton_yaw.py \\
      --pred-playback {OUT_REMOTE}/skeleton_playback_stage3_train_wu_pred.json \\
      --gt-playback {OUT_REMOTE}/skeleton_playback_stage3_train_wu_gt.json \\
      --output {OUT_REMOTE}/skeleton_yaw_stage3_train_wu_pred_vs_gt.mp4 \\
      --report {OUT_REMOTE}/skeleton_yaw_stage3_train_wu_pred_vs_gt.json
...
def main() -> None:
    ...
    with sftp.file("/tmp/run_stage3_aligned_train_wu_viz.sh", "w") as f:
        f.write(REMOTE_SH)
    sftp.close()

    _, o, e = c.exec_command("bash /tmp/run_stage3_aligned_train_wu_viz.sh", timeout=7200)
    ...
    sftp = c.open_sftp()
    names = [
        "stage3_train_wu_eval.json",
        "skeleton_playback_stage3_train_wu_pred.json",
        "skeleton_playback_stage3_train_wu_gt.json",
        "skeleton_yaw_stage3_train_wu_pred.mp4",
        "skeleton_yaw_stage3_train_wu_gt.mp4",
        "skeleton_yaw_stage3_train_wu_pred_vs_gt.mp4",
        "skeleton_yaw_stage3_train_wu_pred_vs_gt.json",
    ]
    for name in names:
