...
| 角色               | 路径                                                                            |
| ---------------- | ----------------------------------------------------------------------------- |
| Manifest         | `test_code/joint_projection/build_aligned_multiview_manifest.py`              |
| 四相机 2D           | `test_code/joint_projection/infer_rtmpose_candidates.py`                      |
| 三角化              | `test_code/joint_projection/process_external_multiview_3d.py`                 |
| 合并               | `test_code/joint_projection/merge_multiview_chunks.py`                        |
| 肢端 GT            | `test_code/joint_projection/replace_limb_mocap_gt.py`                         |
| 头鼻 2D            | `test_code/joint_projection/detect_head_nose_rtmw.py`                         |
| 鼻偏置优化            | `test_code/joint_projection/optimize_multiview_head_nose_offset.py`           |
| 并行重投影渲染          | `test_code/joint_projection/render_nose_offset_parallel.py`                   |
| 头显解码（h265+bytes） | `test_code/joint_projection/render_multiview_to_head.py`（`H265CaptureReader`） |
| 外部刚体外参           | `test_code/joint_projection/external_stereo_rigid_k_extrinsics.json`          |
| 头显刚体外参           | `test_code/joint_projection/head_stereo_rigid_extrinsics.json`                |
| 腕 config         | `test_code/joint_projection/configs/0806_wrist_dual_external_mocap.json`      |
| 踝 config         | `test_code/joint_projection/configs/0806_ankle_dual_external_mocap.json`      |
| 骨长测量模板           | `C:\Users\hand\Desktop\Dataset\骨骼测量记录模板.xlsx`                                 |
| 四相机说明（背景）        | `test_code/joint_projection/0806_双外部双目四相机三角化说明.md`                            |
| 本地总控（参考）         | `test_code/joint_projection/overnight_0806_limb_batch.py`                     |
| 远端一键（参考，默认不启）    | `test_code/joint_projection/run_0806_limb_dataset.sh`                         |
| Delivery 关键点        | `test_code/joint_projection/delivery_keypoints.py`                            |
| Raw playback 导出     | `test_code/joint_projection/export_playback_from_jsonl.py`                    |
| 3D yaw MP4           | `test_code/joint_projection/render_skeleton_yaw_video.py`                     |
