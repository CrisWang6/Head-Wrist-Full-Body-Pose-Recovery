#!/usr/bin/env bash
# Full A→E pipeline for one 0806 limb dataset (wrist|ankle) on gwj.
# Usage: bash run_0806_limb_dataset.sh wrist|ankle
# Stages skip when aligned/detections/triangulation already valid.
set -euo pipefail

NAME="${1:?usage: $0 wrist|ankle}"
...
case "$NAME" in
  ...
  ankle)
    ...
    ;;
  *)
    echo "NAME must be wrist or ankle" >&2
    exit 1
    ;;
esac
...
############################################
run_replace() {
  set_status "E_replace_running"
  ensure_pre_limb
  log "Stage E replace $REPLACE z=${Z_OFFSET_MM}mm mode=per_frame"
  "$PYTHON" "$JP/replace_limb_mocap_gt.py" \
    --results "$FULL/multiview_3d_results_pre_limb.jsonl" \
    --aligned "$DATA_ROOT/aligned_data/aligned_30hz.csv" \
    --output "$FULL/multiview_3d_results_limb_gt.jsonl" \
    --replace "$REPLACE" \
    --z-offset-mm "$Z_OFFSET_MM" \
    --mode per_frame \
    --playback-output "$FULL/skeleton_playback.json" \
    >"$LOGS/replace.log" 2>&1
  cp -f "$FULL/multiview_3d_results_limb_gt.jsonl" "$FULL/multiview_3d_results.jsonl"
  # Optimized 3D yaw (same style as raw).
  "$PYTHON" "$JP/render_skeleton_yaw_video.py" \
    --data "$FULL/skeleton_playback.json" \
    --output "$VIZ/skeleton_3d_optimized_yaw.mp4" \
    --yaw-deg 100 --pitch-deg 18 \
    >"$LOGS/viz_opt_yaw.log" 2>&1 || true
  set_status "E_replace_done"
  tail -n 40 "$LOGS/replace.log" | tee -a "$LOGS/pipeline.log"
}
...
# Main A→E
############################################
log "===== pipeline start $NAME A→E (wrist z=-60 / ankle z=-80) ====="
set_status "starting"
