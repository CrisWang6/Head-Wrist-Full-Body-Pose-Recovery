...
parser.add_argument('--four-color-view', dest='four_color_view', action='store_true', default=True,
                    help="Default mode: open CAM_A..CAM_D as color cameras and combine them in one 2x2 window.")
parser.add_argument('--no-four-color-view', dest='four_color_view', action='store_false',
                    help="Use the original auto-detected camera list and color display behavior.")
parser.add_argument('--four-color-mono', dest='four_color_view', action='store_true',
                    help=argparse.SUPPRESS)
parser.add_argument('--no-four-color-mono', dest='four_color_view', action='store_false',
                    help=argparse.SUPPRESS)
parser.add_argument('--separate-windows', action='store_true',
                    help="Show one OpenCV window per stream instead of combining the main camera streams.")
parser.add_argument('--mosaic-cell-width', type=int, default=640,
                    help="Width of each camera cell in the combined 2x2 preview window.")
...
if int(dai.__version__.split(".", 1)[0]) >= 3 and not any(
    [
        args.stress,
        args.gui,
        args.yolo,
    ...
    cmd = [
        sys.executable,
        str(Path(__file__).resolve().parents[2] / "scripts" / "capture" / "oak_ffc_sync_ar0234.py"),
        "--no-external-trigger",
        "--fps",
    ...
    print("DepthAI v3 detected; using the v3 OAK-FFC 4-camera preview path.")
...
def display_frame_for_mosaic(frame, label, cell_width):
    if frame.ndim == 2:
        frame = cv2.cvtColor(frame, cv2.COLOR_GRAY2BGR)
    else:
        frame = frame.copy()
    scale = cell_width / frame.shape[1]
    resized = cv2.resize(frame, (cell_width, max(1, int(frame.shape[0] * scale))))
    cv2.putText(resized, label, (16, 34), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 255), 2)
    return resized


def make_mosaic(frames, labels, cell_width):
    cells = []
    for label in labels:
        ...
        cells.append(display_frame_for_mosaic(frame, label, cell_width))
    ...
    h = min(cell.shape[0] for cell in cells[:4])
    cells = [cell[:h] for cell in cells[:4]]
    return np.vstack([np.hstack(cells[:2]), np.hstack(cells[2:4])])
...
def get_device_by_id(device_id):
    if not device_id:
        return False, None
    if hasattr(dai.Device, "getDeviceById"):
        return dai.Device.getDeviceById(device_id)
    return dai.Device.getDeviceByMxId(device_id)


def create_pipeline_for_device(device):
    try:
...
success, device = get_device_by_id(args.device)
dai_device_args = []
if success:
...
with dai.Device(*dai_device_args) as device:
    ...
    if args.four_color_view and not args.cameras:
        args.cameras = [
            ['cama', True, False, False],
            ['camb', True, False, False],
            ['camc', True, False, False],
            ['camd', True, False, False],
        ]
        print("Defaulting to 4 color cameras in one view: cama camb camc camd")
    ...
    pipeline = create_pipeline_for_device(device)
    ...
    mosaic_streams = [c for c in cam_list if c in streams][:4]
    mosaic_window = "4 camera color view"
    use_mosaic_window = args.four_color_view and not args.separate_windows and len(mosaic_streams) > 0
    for c in streams:
        q[c] = device.getOutputQueue(name=c, maxSize=4, blocking=False)
        # The OpenCV window resize may produce some artifacts
        if args.resizable_windows and (args.separate_windows or c not in mosaic_streams):
            cv2.namedWindow(c, cv2.WINDOW_NORMAL)
            cv2.resizeWindow(c, (640, 480))
        fps_host[c] = FPS()
        fps_capt[c] = FPS()
    if args.resizable_windows and use_mosaic_window:
        cv2.namedWindow(mosaic_window, cv2.WINDOW_NORMAL)
        cv2.resizeWindow(mosaic_window, (args.mosaic_cell_width * 2, 960))
    ...
    def show_or_buffer(stream_name, frame):
        display_frame = frame
        if use_mosaic_window and stream_name in mosaic_streams:
            latest_mosaic_frames[stream_name] = display_frame
        else:
    ...
    while True:
        for c in streams:
            ...
            if pkt is not None:
                ...
                show_or_buffer(c, frame)
        if use_mosaic_window:
            mosaic = make_mosaic(latest_mosaic_frames, mosaic_streams, args.mosaic_cell_width)
            if mosaic is not None:
                cv2.imshow(mosaic_window, mosaic)
        print("\rFPS:",
